...
import DTRModel, { IDTR } from "@/model/dtrModel"
import { ArrowLeft } from "lucide-react"
import Link from "next/link"

export default async function EditDTR({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const data = await DTRModel.findById(slug)
  ...
  return (
    <div className="max-w-3xl m-auto pt-15 pb-8 flex flex-col gap-4 px-4">
      ...
      <DtrForm action={updateDTR} data={JSON.parse(JSON.stringify(data)) as IDTR} />
    </div>
  )
}